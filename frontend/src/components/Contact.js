          ...
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            ...
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-sm bg-[#0A8EDB]/10 flex items-center justify-center flex-shrink-0">
                  <Phone size={22} className="text-[#0A8EDB]" />
                </div>
                <div>
                  <p className="font-manrope font-semibold text-[#1C1917] text-sm mb-0.5">{t.contact.phoneLabel}</p>
                  <a href="tel:+491632106688" className="font-manrope text-[#57534E] hover:text-[#0A8EDB] transition-colors text-lg" data-testid="contact-phone-link">
                    +49 163 210 66 88
                  </a>
                </div>
              </div>
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-sm bg-[#0A8EDB]/10 flex items-center justify-center flex-shrink-0">
                  <Mail size={22} className="text-[#0A8EDB]" />
                </div>
                <div>
                  <p className="font-manrope font-semibold text-[#1C1917] text-sm mb-0.5">{t.contact.emailLabel}</p>
                  <a href="mailto:info@mk-gebaeudemanagement.de" className="font-manrope text-[#57534E] hover:text-[#0A8EDB] transition-colors" data-testid="contact-email-link">
              ...
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-sm bg-[#0A8EDB]/10 flex items-center justify-center flex-shrink-0">
                  <MapPin size={22} className="text-[#0A8EDB]" />
                </div>
                <div>
          ...
          <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.15 }}>
            <form onSubmit={handleSubmit} data-testid="contact-form" className="bg-[#FDFBF7] border border-[#E7E5E4] rounded-sm p-8 md:p-10 space-y-6">
              <div>
                ...
                <Input id="name" name="name" data-testid="contact-input-name" value={form.name} onChange={handleChange} required className="bg-white border-[#E7E5E4] font-manrope focus-visible:ring-[#0A8EDB]" />
              </div>
              <div>
                <Label htmlFor="email" className="font-manrope font-medium text-[#1C1917] mb-2 block">{t.contact.email} *</Label>
                <Input id="email" name="email" type="email" data-testid="contact-input-email" value={form.email} onChange={handleChange} required className="bg-white border-[#E7E5E4] font-manrope focus-visible:ring-[#0A8EDB]" />
              </div>
              <div>
              ...
              <div>
                ...
                <Textarea id="message" name="message" data-testid="contact-input-message" value={form.message} onChange={handleChange} required rows={5} className="bg-white border-[#E7E5E4] font-manrope focus-visible:ring-[#0A8EDB] resize-none" />
              </div>
              <div className="flex items-start gap-3 p-4 bg-white border border-[#E7E5E4] rounded-sm">
                <input type="checkbox" id="privacy-consent" data-testid="contact-consent-checkbox" checked={consent} onChange={(e) => setConsent(e.target.checked)} required className="mt-1 h-4 w-4 accent-[#0A8EDB] flex-shrink-0 cursor-pointer" />
                <label htmlFor="privacy-consent" className="font-manrope text-xs text-[#57534E] leading-relaxed cursor-pointer">
                  Ich willige ein, dass meine Angaben zur Bearbeitung meiner Anfrage gespeichert werden. Ich kann meine Einwilligung jederzeit mit Wirkung für die Zukunft widerrufen. Weitere Informationen in der{" "}<a href="/datenschutz" className="text-[#0A8EDB] underline hover:text-[#0873B3]" target="_blank" rel="noopener noreferrer">Datenschutzerklärung</a>.<span className="text-red-600">&nbsp;*</span>
                </label>
              </div>
              <Button type="submit" data-testid="submit-contact-form" disabled={loading || !consent} className="w-full bg-[#0A8EDB] hover:bg-[#0873B3] text-white font-manrope font-semibold py-3 rounded-sm transition-all hover:-translate-y-0.5 hover:shadow-md">
                {loading ? (
